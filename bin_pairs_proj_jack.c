...
main( int argc, char *argv[] )
{
    ...
    njack1 = ws_read_ascii( &ws1, w1_file );
    fprintf( stderr, "Reading weight file: %s\n", w2_file );
    njack2 = ws_read_ascii( &ws2, w2_file );
    ...
    bins = ( BINS * ) check_alloc( njack1, sizeof( BINS ) );
    ...
    fprintf( stderr, "DONE! Found %zu pairs in %d files\n", pair_count, nfiles );
    {
        ...
        for( j = 0; j < njack1; j++ ) {
            ...
            /* munge output file to include jack_id */
            {
                ...
                snprintf( fn, len, "%s.%dj%03d.dat", out_file, njack1, j );
            ...
            fprintf( fp, "# w1_total:  %.10g (full %.10g)\n", wtj1[j], ws1.wt );
            fprintf( fp, "# w2_file:   %s\n", w2_file );
            fprintf( fp, "# w2_total:  %.10g (full %.10g)\n", wtj2[j], ws1.wt );
